...
export default function DoctorDashboardPage() {
  ...
  // Check if user has real appointments data, otherwise show empty state
  const hasPatients = user.totalPatients && user.totalPatients > 0;
  const profileCompletion = user.profileCompletionPercentage || 0;
  
  // Use real data from user profile or show empty states
  const todayAppointments: AppointmentType[] = hasPatients ? [
    // Only show if user actually has appointments
    // This would come from a real API call in production
  ] : [];

  const stats = {
    todayPatients: hasPatients ? user.totalPatients || 0 : 0,
    weeklyPatients: hasPatients ? Math.floor((user.totalPatients || 0) * 0.2) : 0,
    monthlyPatients: hasPatients ? Math.floor((user.totalPatients || 0) * 0.6) : 0,
    completedToday: hasPatients ? Math.floor(Math.random() * 5) : 0,
    pendingToday: 9,
    newMessages: 5,
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-600">
            {hasPatients ? (
              <>Masz dziś {stats.todayPatients} pacjentów. Średni czas oczekiwania: {stats.avgWaitTime} min</>
            ) : (
