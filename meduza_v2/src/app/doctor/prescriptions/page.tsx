...
export default function DoctorPrescriptionsPage() {
  ...
  const handleIssuePrescription = async () => {
    ...
    try {
      ...
      // Przygotuj recepty w formacie zgodnym z API
      const prescriptions = medications.map(med => ({
        medicationName: med.name,
        dosage: med.dosage,
        frequency: med.frequency,
        duration: med.duration,
        instructions: med.instructions || "Przyjmować zgodnie z zaleceniami lekarza",
        status: "active"
      }));
      ...
      const response = await fetch("/api/medical-records", {
        ...
        body: JSON.stringify({
          ...
          diagnosis: [],
          symptoms: [],
          treatment: `Recepta na: ${medications.map(m => m.name).join(", ")}`,
      ...
      // Przełącz na zakładkę z wydanymi receptami
      setActiveTab("issued");
    } catch (err) {
      console.error("Error issuing prescription:", err);
      setError((err as Error).message);
      alert("Wystąpił błąd podczas wystawiania recepty");
    } finally {
      setSaving(false);
    }
  };
            ...
            <Card>
              ...
              <CardContent className="space-y-6">
                ...
                <div className="flex justify-end space-x-4 pt-6 border-t">
                  ...
                  <Button onClick={handleIssuePrescription} disabled={saving}>
                    <FileText className="h-4 w-4 mr-2" />
                    {saving ? "Wystawianie..." : "Wystaw receptę"}
                  </Button>
                </div>
